---
title: "Lecture 9: Image Analysis"
...
- **Objective**: Master large-scale automated image analysis

## Why analyze images?

- Abundance of images in our daily lives
...
- Globalization of visual dissemination
- The visual as a social force
- The "pictorial turn" of culture

## Which images to study?

- Masterpieces vs. media images
- Evaluation criteria: social, political, and historical context
...
- Aby Warburg: breaking down barriers between cultures and mediums
- Carl Einstein: critique of colonial biases
- Elevating popular images to the same level as fine arts
...
- The image as transformative and reproductive

## Visual Studies

- Crisis of the visual (Nicholas Mirzoeff); How to analyze representation in overabundance?
...
- Advertising: multidimensionality of analysis
- Brands are built by images
- Qualitative and quantitative approaches
- Cross-cultural comparative studies
- Representations and social constructions

## Examples of traditional quantitative analyses {.smaller}
...
- Comparison of products/services:
  ...
  - Services: more appeal to emotion than products
...
- [ImageNet](https://www.image-net.org/) competition: a decisive turning point
  ...
  - CAPTCHA
- Geoffrey Hinton's team: first victory with neural networks
- 60 million connections, 650,000 neurons on 8 layers
- Spectacular reduction in error rate
...
## Classic function of these algorithms
...
- Measurement of diversity in representation
...
- Case of @wang_kosinski18: sexual orientation prediction
...
- Quantitative analysis of the visual elements present
...
df_images <- df_images %>%
  mutate(
    # Extract the media from the file name
    media = case_when(
...
chemins_images <- list.files(
  ...
  pattern = "\\.(jpg|jpeg|png|PNG)$",    # Filter only image files
...
## Step 1: Creating a dataframe {.smaller}
...
```R
# Extracting media, date, and article type
df_images <- df_images %>%
  mutate(
...
|Analysis Type|Example Prompt|Response Format|
|-------------|--------------|---------------|
...
|General description|"In a migration context, describe this image from the content of a Quebec media article. Pay particular attention to details."|Descriptive paragraph|
...
|Multiple thematic categorization|"This image comes from a Quebec media article. What themes are represented in the image? [list of themes provided]"|Enumeration of themes|

## Step 2: Preparing for analysis tools {.smaller}
...
analyser_image <- function(chemin_image, prompt_texte) {
  ...
  if (file.exists(chemin_image) && 
    ...
    # In a real context, we would make a call to the GPT Vision API here
    # Via the clellm::gpt_vision() package
    
    # Simulated response for demonstration
    reponse <- paste("Analysis of image", basename(chemin_image),
...
2. **Operation**:
   ...
   - Includes the image and the prompt in the request
   - Retrieves and analyzes the response

3. **Returns**: GPT's textual interpretation of the image
...
# Definition of the analysis prompt table
prompts_analyse <- data.frame(
  ...
  prompt = c(
    ...
    "Are there any cultural or political symbols in this image?"
  ),
  stringsAsFactors = FALSE
)

# Selection of a few images for demonstration
...
#| eval: false
  ...
  for (j in 1:nrow(prompts_analyse)) {
    ...
    # Analyze the image with the current prompt
...
# Create the resultats folder if it does not exist
...
## Example of a complete workflow {.smaller}
